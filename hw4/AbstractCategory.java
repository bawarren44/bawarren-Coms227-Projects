...
/**
 * Abstract representation of a scoring category for a dice game such as
 * Yahtzee. Each scoring category has a method <code>isSatisfiedBy</code> that
 * can determine, given a <code>Hand</code>, whether the criteria for the
 * category can potentially be satisfied by the hand. There is also a method
 * <code>getPotentialScore</code> that determines what the potential score would
 * be for a given hand. <strong>These two methods do not modify the state of the
 * category.</strong>
 * <p>
 * A scoring category can also be set to a be <em>filled</em> by calling the
 * <code>fill()</code> method, meaning that a hand being used to fill the
 * category in the game is permanently saved for the category. After being
 * filled, <code>getHand</code> always returns the exact hand that was used to
 * fill the category and <code>getScore</code> always returns the score for that
 * hand.
 * 
 * @author Brad Warren: bawarren@iastate.edu
 */
public abstract class AbstractCategory implements ScoringCategory {
	/**
	 * Determines whether a hand has been filled.
	 */
	private boolean filled = false;

	/**
	 * Hand being used to check scoring categories.
	 */
	private Hand hand;

	/**
	 * Name initialized by the constructor.
	 */
	private String name;

	/**
	 * Constructs a new scoring category with a given name.
	 * 
	 * @param name given name by the user
	 */
	protected AbstractCategory(java.lang.String name) {
		this.name = name;
	}
